...
for (const artifactKey of ['01-organization-root', '02-organization-departments', '03-organization-process-equipment', '04-meters', '05-production-units', '06-production-outputs', '08-meter-readings-2026-08', '09-generation-records']) assert.match(ledgerApiSource, new RegExp(`/templates/demo-park/${artifactKey}\\.xlsx|${artifactKey}`), `缺少天坤集团台账示例 ${artifactKey}`);
...
assert.match(ledgerPageSource, /v-if="config\.template && can\('template'\)"/);
assert.match(ledgerPageSource, /v-if="config\.import && can\('import'\)"/);
assert.match(ledgerPageSource, /v-if="config\.export && can\('export'\)"/);
assert.doesNotMatch(ledgerPageSource, /demoPermission|demoExamples|downloadDemoExample|天坤集团示例/);

  ...
  <ManagementPage title="用能预算">
    ...
    <template v-else>
      <ManagementToolbar :loading="loading" @search="applyFilters" @reset="resetFilters">
        <el-form-item label="开始月份"><el-date-picker v-model="draftFilters.monthStart" type="month" value-format="YYYY-MM" placeholder="开始月份" /></el-form-item>
        <el-form-item label="结束月份"><el-date-picker v-model="draftFilters.monthEnd" type="month" value-format="YYYY-MM" placeholder="结束月份" /></el-form-item>
      ...
      <template v-else>
        ...
        <article class="page-card chart-panel">
          <header class="chart-heading"><div class="heading-with-help"><h2>预算与实际执行对比</h2><HelpIcon label="查看预算执行对比边界" content="只用 active 预算与相同能源类型、组织范围及月份的 active 能耗 normalized_value 对比。图表按单位筛选并使用一条共同数值轴；不做跨能源折标煤或跨单位换算。" /></div><el-select v-model="comparisonUnit" size="small" class="unit-select" placeholder="选择可比单位"><el-option v-for="unit in comparisonUnits" :key="unit" :label="unit" :value="unit" /></el-select></header>
          <PageState v-if="comparisonError" :error="comparisonError" @retry="loadData" />
          <PageState v-else-if="!comparisonRows.length" description="当前筛选下暂无可比较的启用预算" />
          <template v-else>
            ...
            <div class="comparison-chart" @mouseleave="comparisonTooltip = null"><button v-for="row in comparisonRows" :key="comparisonKey(row)" class="comparison-row" type="button" :aria-label="comparisonLabel(row)" @mouseenter="comparisonTooltip = row" @focus="comparisonTooltip = row"><span class="comparison-name">{{ row.periodMonth }} · {{ row.energyTypeName || row.energyTypeCode }} · {{ row.organizationScope }}</span><span class="comparison-bars"><i class="comparison-budget" :style="{ width: `${percentage(row.budgetValue, comparisonMax)}%` }" /><i class="comparison-actual" :style="{ width: `${percentage(row.actualValue, comparisonMax)}%` }" /></span><StatusTag :status="comparisonStatus(row)" :label="row.warningLabel" /></button></div>
            <p v-if="comparisonTooltip" class="chart-tooltip" role="status">{{ comparisonLabel(comparisonTooltip) }}</p>
            <el-table :data="comparisonRows" size="small" class="chart-table"><el-table-column prop="periodMonth" label="月份" width="100" /><el-table-column prop="energyTypeName" label="能源类型" min-width="120" /><el-table-column prop="organizationScope" label="组织范围" min-width="130" /><el-table-column label="预算" min-width="115"><template #default="{ row }">{{ formatNumber(row.budgetValue) }} {{ row.unit }}</template></el-table-column><el-table-column label="实际" min-width="115"><template #default="{ row }">{{ formatNumber(row.actualValue) }} {{ row.actualUnit || row.unit }}</template></el-table-column><el-table-column label="差异" min-width="110"><template #default="{ row }">{{ formatNumber(row.variance) }}</template></el-table-column><el-table-column label="使用率" min-width="100"><template #default="{ row }">{{ formatPercent(row.usageRate) }}</template></el-table-column><el-table-column label="状态" min-width="115"><template #default="{ row }"><StatusTag :status="comparisonStatus(row)" :label="row.warningLabel" /></template></el-table-column></el-table>
          </template>
        </article>
...
const comparisonUnits = computed(() => [...new Set(comparison.value.filter((row) => row.unit && (!row.actualUnit || row.actualUnit === row.unit)).map((row) => row.unit))]);
const comparisonRows = computed(() => comparison.value.filter((row) => row.unit === comparisonUnit.value && (!row.actualUnit || row.actualUnit === row.unit)));
const trendMax = computed(() => Math.max(...monthlyRows.value.map((row) => numberValue(row.budgetValue)), 1)); const structureMax = computed(() => Math.max(...structureRows.value.map((row) => numberValue(row.budgetValue)), 1)); const comparisonMax = computed(() => Math.max(...comparisonRows.value.flatMap((row) => [numberValue(row.budgetValue), numberValue(row.actualValue)]), 1));
...
function comparisonStatus(row) { return row.warningLevel === 'exceeded' ? 'inactive' : row.warningLevel === 'nearing' ? 'candidate-by-meter' : row.warningLevel === 'missing_budget' ? 'missing' : 'active'; }
function comparisonLabel(row) { return `${row.periodMonth} ${row.energyTypeName || row.energyTypeCode} ${row.organizationScope}：预算 ${formatNumber(row.budgetValue)} ${row.unit}，实际 ${formatNumber(row.actualValue)} ${row.actualUnit || row.unit}，差异 ${formatNumber(row.variance)}，使用率 ${formatPercent(row.usageRate)}，${row.warningLabel}`; }
...
async function loadData() {
  ...
  if (comparisonResult.ok) { comparison.value = comparisonResult.value.data || []; comparisonError.value = ''; if (!comparisonUnits.value.includes(comparisonUnit.value)) comparisonUnit.value = comparisonUnits.value[0] || ''; } else { comparison.value = []; comparisonError.value = requestError(comparisonResult); }
}

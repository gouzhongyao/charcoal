...
import { clearDemoContexts } from '@/api/demoData';
...
export const useUserStore = defineStore('user', {
  ...
  actions: {
    ...
    async logout(remote = true) { try { if (remote && this.token) await request({ method: 'post', url: '/auth/logout' }); } finally { this.token = ''; this.profile = null; this.ready = false; localStorage.removeItem(TOKEN_KEY); clearDemoContexts(); } }
  }
});

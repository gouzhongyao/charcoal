...
function buildContentDisposition(fileName, fallbackName) {
  const fallback = String(fallbackName || fileName).replace(/[^A-Za-z0-9._-]+/g, '-') || 'jiliang-qiju.xlsx';
...
router.get('/export', authenticate, requirePermission('ledger:meters:export'), asyncHandler(async (req, res) => {
  ...
  res.setHeader('Content-Disposition', buildContentDisposition(result.fileName, `jiliang-qiju.${result.format}`));

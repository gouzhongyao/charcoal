...
function buildContentDisposition(fileName, fallbackName) {
  const fallback = String(fallbackName || fileName).replace(/[^A-Za-z0-9._-]+/g, '-') || 'energy-budgets.xlsx';
...
router.get('/export', authenticate, requirePermission('energy:budget:export'), asyncHandler(async (req, res) => {
  ...
  res.setHeader('Content-Disposition', buildContentDisposition(result.fileName, `energy-budgets.${result.format}`));

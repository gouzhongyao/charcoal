...
const requireEnergyRecordView = requireAnyPermission('energy:records:view', 'energy-records:view');
...
router.get('/export', requireEnergyRecordView, asyncHandler(async (req, res) => {
  ...
  res.setHeader('Content-Type', result.contentType);
  res.setHeader('Content-Disposition', buildContentDisposition(result.fileName, `energy-records.${result.format}`));
  res.setHeader('Content-Length', String(result.body.length));
  res.setHeader('X-Export-Row-Count', String(result.rowCount));
  res.status(200).send(result.body);
}));

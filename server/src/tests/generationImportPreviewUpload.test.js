...
function postMultipart(port, filename, fileContent, mimeType = 'text/csv') {
...
(async () => {
  ...
  try {
    ...
    await runWithMaintenance('test-maintenance', async () => {
      const blocked = await postMultipart(port, 'generation-maintenance.csv', validCsv);
    ...
    const success = await postMultipart(port, 'generation-success.csv', validCsv);
    ...
    const invalid = await postMultipart(port, 'generation-invalid.xlsx', 'not a real xlsx file', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');

...
try {
  ...
  try {
    ...
    assert.strictEqual(dashboardMenu.menuName, '驾驶舱', '新库菜单名称必须直接使用“驾驶舱”。');
    ...
    ) VALUES ('dashboard_reader', '驾驶舱查看员', 'active', ?, ?)`).run(now, now).lastInsertRowid;
    setupDb.prepare(`INSERT INTO sys_role_menus (role_id, menu_id, created_at)
      VALUES (?, ?, ?)`).run(roleId, dashboardMenuId, now);
    setupDb.prepare("UPDATE sys_menus SET menu_name = '工作台', updated_at = ? WHERE id = ?")
  ...
  try {
    ...
    assert.strictEqual(migratedMenu.menuName, '驾驶舱');
  ...
  try {
    ...
    assert.strictEqual(repeatedMenu.menuName, '驾驶舱');
    assert.strictEqual(

...
const PREDICTION_CONFIG_EXPORT_FIELDS = Object.freeze([
  ...
  { key: 'meterName', header: '计量器具名称' }, { key: 'sourceBatchId', header: '能耗批次ID' },
...
function normalizeTrainingFilters(payload = {}) {
  return {
    ...
    organizationUnitCode: normalizeText(payload.organizationUnitCode || payload.organization_unit_code),
    organizationUnitId: normalizePositiveInteger(payload.organizationUnitId || payload.organization_unit_id, 'organizationUnitId'),
    meterCode: normalizeText(payload.meterCode || payload.meter_code),
    meterDeviceId: normalizePositiveInteger(payload.meterDeviceId || payload.meter_device_id, 'meterDeviceId'),
...
function normalizeConfigPayload(input = {}, options = {}) { const existing = options.existing || {}; const merged = { ...existing, ...input, organizationUnitCode: input.organizationUnitCode ?? existing.organizationUnitCode, organizationUnitId: input.organizationUnitId ?? existing.organizationUnitId, meterCode: input.meterCode ?? existing.meterCode, meterDeviceId: input.meterDeviceId ?? existing.meterDeviceId, sourceBatchId: input.sourceBatchId ?? existing.sourceBatchFilterId ?? existing.sourceBatchId }; const normalized = normalizePredictionPayload(merged); const status = normalizeConfigStatus(input.status, existing.status || 'draft'); if (status === 'archived' && !options.allowArchived) throw badRequest('请通过状态接口归档预测配置。', { code: 'PREDICTION_CONFIG_ARCHIVE_ROUTE_REQUIRED' }); if (normalized.name.length > 200 || (normalized.note && normalized.note.length > 1000)) throw badRequest('预测配置名称或备注长度超出限制。', { code: 'PREDICTION_CONFIG_TEXT_TOO_LONG' }); return { ...normalized, status }; }
function mapConfigRow(row) { return row ? { ...row, windowSize: row.windowSize === null ? null : Number(row.windowSize), sourceBatchId: row.sourceBatchId === null ? null : Number(row.sourceBatchId), organizationUnitId: row.organizationUnitId === null ? null : Number(row.organizationUnitId), meterDeviceId: row.meterDeviceId === null ? null : Number(row.meterDeviceId) } : null; }
...
function createRunFromConfig(configIdRaw) { const configId = normalizePositiveInteger(configIdRaw, 'configId'); const db = openDatabase(); try { return db.transaction(() => { const config = getPredictionConfigById(db, configId); if (config.status === 'archived') throw badRequest('已归档预测配置不可运行；请复制为新草稿。', { code: 'PREDICTION_CONFIG_ARCHIVED' }); const payload = normalizePredictionPayload(config); const snapshot = { configId: config.id, config: { name: config.name, note: config.note, energyTypeCode: config.energyTypeCode, organizationUnitId: config.organizationUnitId, organizationUnitCode: config.organizationUnitCode, meterDeviceId: config.meterDeviceId, meterCode: config.meterCode, sourceBatchId: config.sourceBatchFilterId, trainStartMonth: config.trainStartMonth, trainEndMonth: config.trainEndMonth, predictStartMonth: config.predictStartMonth, predictEndMonth: config.predictEndMonth, algorithm: config.algorithm, windowSize: config.windowSize } }; return runNormalizedPrediction(db, payload, snapshot); })(); } finally { db.close(); } }
